...
void WriteNpzExamples() {
	...
	// 创建测试数据
	const std::vector<float> float_data = {1.0f, 2.0f, 3.0f, 4.0f};
	const std::vector<int> int_data = {10, 20, 30, 40, 50};
	const std::vector<double> double_data = {1.1, 2.2, 3.3, 4.4};

	const std::vector<size_t> shape2d = {2, 2};
	const std::vector<size_t> shape1d_int = {5};
	const std::vector<size_t> shape1d_double = {4};
	const std::string npz_path = fs::path(EXAMPLES_DIR) / "res" / "cpp_saved.npz";

	try {
		// 首先使用写入模式创建文件
		{
			npy::NpzWriter writer(npz_path, npy::NpzWriter::Mode::W);
		...
		// 然后使用追加模式添加更多数据
		{
			npy::NpzWriter writer(npz_path, npy::NpzWriter::Mode::A);
...
int main() {
	...
	const std::string npz_path = fs::path(EXAMPLES_DIR) / "res" / "cpp_saved.npz";
	const std::string scripts_path = fs::path(EXAMPLES_DIR) / "scripts" / "NpypyExamples.py";
